// Calculadora RP (genérica): edite catalog.json para seus itens.
// Template para itens de RP. Use de forma responsável.
...
function calcTotals(){
  const t = { molas:0, canos:0, gatilhos:0, pratas:0, bronzes:0, valor:0 };
  for (const item of state.catalog){
    ...
    t.molas += (r.molas||0) * q;
    t.canos += (r.canos||0) * q;
    t.gatilhos += (r.gatilhos||0) * q;
    t.pratas += (r.pratas||0) * q;
    t.bronzes += (r.bronzes||0) * q;
...
function renderTotals(){
  const t = calcTotals();
  ["molas","canos","gatilhos","pratas","bronzes"].forEach(k => {
  ...
  rows.forEach((tr, idx) => {
    ...
    const q = Number(state.qty[item.id] || 0);
    const r = item.recipe || {};
    const map = { molas:r.molas||0, canos:r.canos||0, gatilhos:r.gatilhos||0, pratas:r.pratas||0, bronzes:r.bronzes||0 };
...
function renderTable(){
  ...
  for (const item of state.catalog){
    ...
    const cols = [
      ["molas", r.molas||0],
      ["canos", r.canos||0],
      ["gatilhos", r.gatilhos||0],
      ["pratas", r.pratas||0],
      ["bronzes", r.bronzes||0],
    ];
    for (const [k, unit] of cols){
...
function buildReceiptText(){
  ...
  if (!state.onlyValue){
    lines.push("🧾 Totais:");
    lines.push(`• Molas: ${fmt(t.molas)}`);
    lines.push(`• Canos: ${fmt(t.canos)}`);
    lines.push(`• Gatilhos: ${fmt(t.gatilhos)}`);
    lines.push(`• Pratas: ${fmt(t.pratas)}`);
    lines.push(`• Bronzes: ${fmt(t.bronzes)}`);
    lines.push("");
  }
...
async function init(){
  ...
  els.btnCopyTotals.addEventListener("click", async () => {
    ...
    const text = state.onlyValue
      ...
      : `Molas: ${fmt(t.molas)} | Canos: ${fmt(t.canos)} | Gatilhos: ${fmt(t.gatilhos)} | Pratas: ${fmt(t.pratas)} | Bronzes: ${fmt(t.bronzes)} | Valor: ${fmtK(t.valor)}`;
